...
<pre><code>## 프로젝트 구조 ```plaintext event-reward-system/backend ├── auth-server/ # 인증 및 유저/역할 관리 서비스 ├── gateway-server/ # API 게이트웨이 및 인증/인가 프록시 ├── event-server/ # 이벤트 등록 및 보상 처리 서비스 ├── docker-compose.yml # 전체 서비스 구성용 Docker Compose ├── .env # 루트 공통 환경 변수 설정 └── README.md # 프로젝트 설명 파일 (현재 문서) ``` </code></pre>
